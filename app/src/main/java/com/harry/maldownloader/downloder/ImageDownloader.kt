...
class ImageDownloader(private val context: Context) {
    ...
    suspend fun downloadImageForEntry(entry: AnimeEntry): AnimeEntry = withContext(Dispatchers.IO) {
        ...
        val placeholder = createPlaceholderBitmap(entry.title, 300, 400)  // Common anime poster size
    ...
    private suspend fun fetchImageUrl(seriesId: Int): String? {
        repeat(3) { attempt ->
            try {
                ...
                if (response.isSuccessful && response.body()?.data?.images?.jpg?.image_url != null) {
                    return response.body()!!.data.images.jpg.image_url
                }
            } catch (e: Exception) {
    ...
    private fun createPlaceholderBitmap(title: String, width: Int, height: Int): Bitmap {
        ...
        canvas.drawColor(Color.DKGRAY)  // Dark theme fallback
        ...
        paint.getTextBounds(title, 0, min(title.length, 20), textBounds)  // Truncate long titles
        val x = width / 2f
